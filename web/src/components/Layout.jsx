  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50/60 selection:bg-teal-100 selection:text-teal-900">
      ...
      <header className="sticky top-0 z-30 glass-header">
        <div className="mx-auto flex max-w-7xl h-16 items-center justify-between gap-6 px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <span className="hidden sm:inline-flex text-2xs font-bold uppercase tracking-wider text-teal-800 bg-teal-50 border border-teal-200/80 px-2 py-0.5 rounded-full">
              {user?.role?.toLowerCase()}
            </span>
          </div>

          {/* Center: Spacious Navigation Pills */}
          ...
          {/* Right: Unified Profile & Role Menu */}
          <div className="flex items-center gap-3 shrink-0">
            ...
            <div className="relative">
              ...
              >
                ...
                <div className="hidden sm:block text-left text-xs leading-tight">
                  <p className="font-bold text-slate-800 truncate max-w-[110px]">{user?.fullName?.split(' ')[0]}</p>
                </div>
              ...
              {profileDropdown && (
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-64 rounded-2xl border border-slate-200 bg-white p-2.5 shadow-xl z-50 animate-in fade-in zoom-in-95">
                    {/* User Info Header */}
                    ...
                    {/* Role Switcher */}
                    <p className="px-3 py-1 text-2xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1">
                    ...
                    <div className="mt-1 space-y-0.5">
                      {DEMO_USERS.map((d) => (
                        <button
                          ...
                          className={`w-full flex items-center justify-between rounded-xl px-3 py-1.5 text-left text-xs transition-colors cursor-pointer ${
                            user?.email === d.email
                    ...
                    <div className="mt-2 pt-2 border-t border-slate-100">
                      <button
                        ...
                        className="w-full flex items-center gap-2 rounded-xl px-3 py-1.5 text-left text-xs font-semibold text-red-600 hover:bg-red-50 transition-colors cursor-pointer"
                      >
      ...
      {integrations && !integrations.email?.configured && (
        <div className="border-b border-amber-200/80 bg-amber-50/90 px-4 py-1.5 text-center text-xs text-amber-900 flex items-center justify-center gap-2">
          <Mail className="w-3.5 h-3.5 text-amber-700 shrink-0" />
          <span>
            <strong>Demo Mode:</strong> Outbox emails saved locally to <code className="font-mono bg-amber-100/80 px-1 py-0.5 rounded text-amber-950 font-semibold">server/.mailbox</code>
          </span>
        </div>
      )}
      ...
      <footer className="mt-auto border-t border-slate-200/80 bg-white/60 py-6 text-center text-xs text-slate-400">
        <div className="mx-auto max-w-7xl px-4 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-2 text-slate-500">
            ...
            <span>CarePulse — Healthcare Appointment &amp; AI Follow-up Manager</span>
          </div>
          <p className="text-slate-400">
            Postgres Advisory Locks · Claude AI Summaries
          </p>
        </div>
      </footer>
    </div>
